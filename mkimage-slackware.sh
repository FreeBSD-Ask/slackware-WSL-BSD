...
if $(file ${CACHEFS}/isolinux/initrd.img | grep -wq XZ) ; then
	xzcat "${CACHEFS}/isolinux/initrd.img" | cpio -i -d -v -m --null
else
	zcat "${CACHEFS}/isolinux/initrd.img" | cpio -i -d -v -m --null
fi

[ -h ${ROOTFS}/cdrom ] && rm ${ROOTFS}/cdrom
...
mount -t nullfs ${CACHEFS} ${ROOTFS}/cdrom
mount -t devfs devfs ${ROOTFS}/dev
mount -o ro -t linsysfs linsysfs ${ROOTFS}/sys
mount -t linprocfs linprocfs ${ROOTFS}/proc
...
sed -i '' 's/DIALOG=on/DIALOG=off/g' etc/slackpkg/slackpkg.conf
sed -i '' 's/POSTINST=on/POSTINST=off/g' etc/slackpkg/slackpkg.conf
sed -i '' 's/SPINNING=on/SPINNING=off/g' etc/slackpkg/slackpkg.conf
